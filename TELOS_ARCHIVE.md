...
**Why this file exists.** `TELOS_STATE.md` is the handoff document and is
meant to be read first. Its "Settled — don't re-litigate" section had grown
to 730 of its 1,063 lines, which made the thing you are supposed to read
first the thing nobody finishes.

So the decisions stayed and the reasoning moved. `TELOS_STATE.md` lists every
settled decision as a one-line index; this file holds the paragraph behind
each one — why it was decided that way, what it cost to find out, and what
would break if it were undone.

**Nothing here is stale by virtue of being here.** "Archive" means settled,
not superseded. If something in this file is contradicted by the code or by
`TELOS_STATE.md`, trust those and fix this.
